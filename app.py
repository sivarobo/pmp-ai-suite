...
return "PART I (Q1-14): 1 Mark Questions ONLY. Q1-3 Synonyms, Q4-6 Antonyms, Q7-14 Grammar Grids. PART II: Poetry Appreciation, Poetic Devices, Grammar Transformation (Voice, Speech, Punctuation, Simple/Compound/Complex), Road Map."
...
subject_blueprint_rules = f"MATHEMATICS RULES: [DRAW_TRIANGLE: AB=5cm] tags. {calculate_math_weightage_from_csv(lessons_list)}"
else:
lang_instruction = "Language: Pure TAMIL language only."
header_format = "பகுதி [ROMAN_NUM]"
option_format = "Options marker: அ) , ஆ) , இ) , ஈ)"
subject_blueprint_rules = ""
return f"Setter constraints: Subject: {subject}, Lessons: {lessons_str}, Type: {exam_type}, Marks: {total_marks}, Time: {exam_time}, Mode: {exam_mode}. Blueprint: {blueprint_desc}. Format: {header_format}, Options: {option_format}. {lang_instruction}. {subject_blueprint_rules} === ANSWER KEY ==="
...
selected_lessons = st.multiselect("Select Chapters / அத்தியாயங்கள்", df[df['Subject'] == subject_val]['Lesson'].unique())
st.markdown("---")
...
blueprint_desc = f"Part I: {p1_ask} Qs. Part II: Answer {p2_ask}/{p2_get}. Part III: Answer {p3_ask}/{p3_get}. Part IV: Answer {p4_ask}/{p4_get}."
...
st.success("✅ வினாத்தாள் தயாராக உள்ளது!")
